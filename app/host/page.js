...
export default function HostPage() {
  ...
  async function nextStep() {
    // From 'question' → 'reveal'. From 'reveal' → next question (or finished).
    if (!game) return;
  ...
  const currentQ =
    game.current_question >= 0 ? QUESTIONS[game.current_question] : null;
  ...
  return (
    <main className="flex-1 flex flex-col p-6 bg-gradient-to-br from-indigo-600 to-purple-700 text-white">
      <div className="max-w-4xl w-full mx-auto flex-1 flex flex-col gap-6">
        {game.status === "lobby" && (
          <Lobby game={game} players={players} onStart={startGame} />
        )}
        ...
        {game.status === "reveal" && currentQ && (
          <RevealView
            q={currentQ}
            players={players}
            isLast={game.current_question === QUESTIONS.length - 1}
            onNext={nextStep}
          />
        )}
...
function Lobby({ game, players, onStart }) {
  ...
  return (
    <div className="flex-1 flex flex-col items-center justify-center text-center gap-8">
      <div>
        ...
        <p className="text-2xl font-mono font-semibold">{joinUrl}</p>
      </div>
      <div>
...
function QuestionView({ game, q, timeLeft, answeredCount, playerCount, onNext }) {
  const seconds = Math.ceil(timeLeft / 1000);
  return (
    <div className="flex-1 flex flex-col gap-6">
      <div className="flex justify-between items-center text-indigo-100">
        <span>Question {game.current_question + 1} / {QUESTIONS.length}</span>
